(function(){
  ...
  const historyManager = {
    ...
    render() {
      ...
      if (badge) badge.style.display = this.items.length ? 'flex' : 'none';
      ...
      list.innerHTML = this.items.map((item, i) => `
        <div class="history-item" data-i="${i}">
          <div class="hi-icon" style="background:${item.catColor || 'var(--accent-soft)'};color:${item.catAccent || 'var(--accent)'}">
            ${item.icon || '📄'}
          </div>
          <div class="hi-info">
            <div class="hi-name" title="${item.toolTitle || ''}">${item.outputName || 'output'}</div>
            <div class="hi-meta">${item.toolTitle || ''} · ${item.size || ''} · ${item.timeAgo || 'just now'}</div>
          </div>
          ${item.blobUrl ? `<button class="hi-dl" data-url="${item.blobUrl}" data-name="${item.outputName}">Download</button>` : ''}
        </div>
      `).join('');
      
      list.querySelectorAll('.hi-dl').forEach(btn => {
  ...
  (function createHistoryPanel() {
    ...
    panel.querySelector('.clear-btn').addEventListener('click', () => historyManager.clear());
